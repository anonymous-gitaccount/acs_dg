...
class CooperativeAStar:
    def __init__(self, dataset,X, node_index, model, eta, tau,hop_neighbor,bounds=(0, 1)):
        #within the function, X is no longer a tensor. feel free to use it as a numpy
    ...
    def target_pixels(self, X, hop_neighbor_list):
        ...
        for idx in range(len(manipulated_matrices)):
            ...
            heuristic = (p_max - p_2dn_max) *5.8* self.TAU  # heuristic value determines Admissible (lb) or not (ub)
            if self.hop>0:
                heuristic = (p_max - p_2dn_max) *5.8*math.sqrt(len(self.NEIGHBORS))* self.TAU  # heuristic value determines Admissible (lb) or not (ub)
    ...
    def apply_atomic_manipulation(self, X, atomic):
        ...
        #Uncomment to remove permutations
        '''
    ...
    def play_game(self, node_index):
        ...
        while self.cal_distance(self.X, new_X) <= self.DIST_VAL and new_label == self.LABEL:
            ...
            #print(self.explored)
            #uncomment: remove permutation
            #self.add_to_exp(self.ADV_MANIPULATION)
            #print(self.DIST_EVALUATION)
            ...
            print(self.DIST_EVALUATION.get(self.ADV_MANIPULATION))
