...
class MCTSCooperative:
    ...
    def __init__(self, data_set, model, node_index, X, tau, eta,hop):
        ...
        self.tau = tau
        self.eta = eta
        feature_extraction = FeatureExtraction_lb(dataset=data_set)
        self.PARTITIONS,self.NEIGHBORS = feature_extraction.get_partitions(self.node_index, self.hop,num_partition=10)
    ...
    def sampleNext(self, k, newResult, dist):
        ...
        #print("newClass{}, newConfident{}", newClass,newConfident)
        #print("dist is", dist)
        '''
        if newClass != self.originalClass and newConfident > effectiveConfidenceWhenChanging:
            ...
            if self.bestCase[0] > dist:
                ...
                path0 = "%s_pic/%s_currentBest_%s.png" % (self.data_set, self.X_index, self.numConverge)
