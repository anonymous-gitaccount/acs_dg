...
def lowerbound(dataset_name, node_index, game_type, eta, tau,hop_neighbor):
    ...
    (label,confidence,margin)=NN.predict_with_margin(X)
    label=label[node_index]
    confidence=confidence[node_index]
    label_str = NN.get_label(int(label))
    margin=margin[node_index]
    print("Working on input with index %s, whose class is '%s' and the confidence is %s with margin %s."
          % (node_index, label_str, confidence,margin))
