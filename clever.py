...
radius=5
count=0
for idx in range(len(data.test_mask)):
    if idx in skip_x_idx or data.test_mask[idx]==False:
        continue
    #print("node ",idx)
    ...
    for itr in range(iterations):
        ...
        #randomly select a point
            neighbors, _, _, _=k_hop_subgraph(node_idx=idx, num_hops=0, edge_index=data.edge_index)
    ...
    for target_class in targeted_classes_node:
        ...
        loc_x_0.append(-loc)
    print("lipschitz estimate for the node {} is {}".format(idx, max(loc_x_0)))
